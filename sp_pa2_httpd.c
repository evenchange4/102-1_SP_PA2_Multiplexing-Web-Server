...
main( int argc, char** argv )
{
    ...
    while (1){
        ...
        FD_ZERO(&writefds);  // clear the socket set
        FD_SET(server.listen_fd, &readfds); // add server socket to set
        ...
        // fprintf(stderr, "getdtablesize() = %i\n", maxfd);
        activity = select( maxfd, &readfds, &writefds, NULL, NULL);
        ...
        for (i = 0; i < max_clients; i++) {
            ...
            if (FD_ISSET(conn_fd , &readfds)){
                ...
                fprintf(stderr, "conn_fd %i is ready!!!\n", conn_fd);
                while(1){
...
static void
init_request( http_request* reqP )
{
    reqP->conn_fd = -1;
...
static void  
free_request( http_request* reqP )
{
    if ( reqP->buf != NULL ) 
    {
	free( reqP->buf );
...
//
static int
read_header_and_file( http_request* reqP, int *errP )
{
...
static void
add_to_buf( http_request *reqP, char* str, size_t len )
{ 
    char** bufP = &(reqP->buf);
...
static char* 
get_request_line( http_request *reqP )
{ 
    int begin;
    char c;
...
static void
init_http_server( http_server *svrP, unsigned short port )
{
